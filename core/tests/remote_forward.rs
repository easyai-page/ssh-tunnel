...
async fn setup() -> (
    support::TestServerHandle,
    ssh_tunnel_core::ssh::client::Connection,
    std::net::SocketAddr,
) {
...
async fn remote_forward_pipes_data() {
    ...
    start_remote_forward(&fwd, &conn.handle, &conn.remote_forwards)
        .await
        .unwrap();
    ...
    let mut client = tokio::net::TcpStream::connect(("127.0.0.1", assigned as u16))
        .await
        .unwrap();
...
async fn stop_remote_forward_cleans_up() {
    ...
    stop_remote_forward(&applied, &conn.handle, &conn.remote_forwards)
        .await
        .unwrap();
