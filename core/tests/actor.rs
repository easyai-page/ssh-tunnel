...
use ssh_tunnel_core::model::{
    AuthMethod, Forward, ForwardKind, ForwardStatus, Server, ServerStatus,
};
...
async fn connect_and_disconnect() {
    ...
    let kh = Arc::new(Mutex::new(KnownHosts::new(
        tempfile::tempdir().unwrap().path().join("kh"),
    )));
...
async fn no_reconnect_when_disabled() {
    ...
    while let Ok(Ok(ev)) = tokio::time::timeout(deadline - std::time::Instant::now(), rx.recv())
        .await
        .map_err(|_| ())
    {
        if let TunnelEvent::ServerStatus {
...
async fn start_forward_during_reconnecting_connects_once() {
    ...
    {
        if let TunnelEvent::ServerStatus {
            status: ServerStatus::Connecting | ServerStatus::Connected,
            ..
        } = ev
        {
            second_connect = true;
    ...
    assert!(
        !second_connect,
        "Reconnecting 期间 StartForward 隐式连上后出现了二次连接事件"
    );
